...
export default async function PagesPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return (<><AdminPageHeader title="Pages" eyebrow="Contenu" description={`${data.meta.total} élément(s)`} action={{ href: '/admin/pages/nouveau', label: 'Nouveau / Nouvelle' }} /><PagesTable data={data.data} pagination={data.meta} /></>);
}
