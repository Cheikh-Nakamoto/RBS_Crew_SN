...
export default async function ProjetsPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return (<><AdminPageHeader title="Projets" eyebrow="Contenu" description={`${data.meta.total} élément(s)`} action={{ href: '/admin/projets/nouveau', label: 'Nouveau / Nouvelle' }} /><ProjetsTable data={data.data} pagination={data.meta} /></>);
}
