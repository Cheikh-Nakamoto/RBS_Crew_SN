...
export function ProjetsTable({ data, pagination }: { data: any[]; pagination: PaginatedMeta }) {
  ...
  const confirmDelete = () => { if (!deleteId) return; startDeleting(async () => { try { await deleteProjets(deleteId); toast.success('Élément supprimé'); setDeleteId(null); } catch { toast.error('Erreur'); } }); };
