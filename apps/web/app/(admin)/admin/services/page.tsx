...
export default async function ServicesPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return (<><AdminPageHeader title="Services" eyebrow="Contenu" description={`${data.meta.total} élément(s)`} action={{ href: '/admin/services/nouveau', label: 'Nouveau / Nouvelle' }} /><Suspense><ServicesTable data={data.data} pagination={data.meta} /></Suspense></>);
}
