...
function getColumns(onDelete: (id: string) => void): ColumnDef<any>[] {
  return [
    ...
    { id: 'actions', header: '', cell: ({ row }: { row: { original: { id: string } } }) => <DataTableRowActions id={row.original.id} basePath="/admin/artistes" onDelete={onDelete} />, size: 48 },
  ];
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function ArtistesTable({ data, pagination }: { data: any[]; pagination: PaginatedMeta }) {
  ...
  const confirmDelete = () => { if (!deleteId) return; startDeleting(async () => { try { await deleteArtistes(deleteId); toast.success('Élément supprimé'); setDeleteId(null); } catch { toast.error('Erreur'); } }); };
