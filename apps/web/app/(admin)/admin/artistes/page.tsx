...
export default async function ArtistesPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return (<><AdminPageHeader title="Artistes" eyebrow="Contenu" description={`${data.meta.total} élément(s)`} action={{ href: '/admin/artistes/nouveau', label: 'Nouveau / Nouvelle' }} /><Suspense><ArtistesTable data={data.data} pagination={data.meta} /></Suspense></>);
}
