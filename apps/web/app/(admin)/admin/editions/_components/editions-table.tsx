...
function getColumns(onDelete: (id: string) => void): ColumnDef<any>[] {
  return [
    ...
    { id: 'actions', header: '', cell: ({ row }: { row: { original: { id: string } } }) => <DataTableRowActions id={row.original.id} basePath="/admin/editions" onDelete={onDelete} />, size: 48 },
  ];
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function EditionsTable({ data, pagination }: { data: any[]; pagination: PaginatedMeta }) {
