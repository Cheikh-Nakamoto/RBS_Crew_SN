...
export default async function EditionsPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return (<><AdminPageHeader title="Éditions festival" eyebrow="Contenu" description={`${data.meta.total} élément(s)`} action={{ href: '/admin/editions/nouveau', label: 'Nouveau / Nouvelle' }} /><EditionsTable data={data.data} pagination={data.meta} /></>);
}
