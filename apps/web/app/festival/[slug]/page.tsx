...
export default async function FestivalEditionPage({ params }: { params: { slug: string } }) {
  let edition: FestivalEditionItem | null = null;
  let fetchError = false;

  try {
    const data = await api
      .get(`festivals/${params.slug}`, { headers: { 'Accept-Language': 'fr' }, next: { revalidate: 3600 } })
      .json<ApiResponse<FestivalEditionItem>>();
    edition = data?.data ?? null;
  } catch (err) {
