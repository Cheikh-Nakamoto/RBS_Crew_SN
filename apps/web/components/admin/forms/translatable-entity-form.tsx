...
export interface TranslatableEntityFormProps {
  mode: 'create' | 'edit';
  backHref: string;
  entityLabel: string;
  showImage?: boolean;
...
export function TranslatableEntityForm({
  mode,
  backHref,
  entityLabel,
  showImage = false,
  ...
  onUpdate,
  extraFields,
}: TranslatableEntityFormProps) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();

  const form = useForm<SimpleTranslatableFormValues>({
    resolver: zodResolver(simpleTranslatableSchema) as Resolver<SimpleTranslatableFormValues>,
    defaultValues: {
  ...
  const onSubmit = (data: SimpleTranslatableFormValues) => {
    const payload = {
      ...data,
        ...
        <div className="flex items-center justify-between gap-4 sticky top-0 z-10 -mx-6 px-6 py-4 bg-[#080808]/90 backdrop-blur-xl border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-black text-white">
              {mode === 'create' ? `Nouveau / Nouvelle ${entityLabel}` : `Modifier ${entityLabel}`}
            </h1>
          </div>
          <Button
            type="submit"
