...
export function ImageGallery({ images = defaultImages }: { images?: ImageData[] }) {
  ...
  return (
    <div className="flex items-center justify-center font-sans w-full min-h-[420px] sm:min-h-[500px] md:min-h-[600px] h-full py-8 sm:py-12 relative group">
      ...
      <button
        className="absolute left-4 sm:left-[calc(50%-300px-50px)] top-1/2 z-[101] flex h-12 w-12 sm:h-16 sm:w-16 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border-2 border-white/20 bg-white/10 backdrop-blur-sm shadow-[0_8px_32px_rgba(0,0,0,0.12)] outline-none transition-all duration-300 ease-out hover:scale-110 hover:bg-white/20 hover:border-white/40 hover:shadow-[0_12px_48px_rgba(0,0,0,0.18)] active:scale-95 focus-visible:ring-4 focus-visible:ring-white/50 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100"
        onClick={prev}
        disabled={disabled}
        aria-label="Previous Image"
      >
        <ChevronLeft className="w-6 h-6 sm:w-8 sm:h-8 text-white transition-transform duration-300 group-hover:-translate-x-0.5" />
      </button>

      <button
        className="absolute right-4 sm:right-[calc(50%-300px-50px)] top-1/2 z-[101] flex h-12 w-12 sm:h-16 sm:w-16 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border-2 border-white/20 bg-white/10 backdrop-blur-sm shadow-[0_8px_32px_rgba(0,0,0,0.12)] outline-none transition-all duration-300 ease-out hover:scale-110 hover:bg-white/20 hover:border-white/40 hover:shadow-[0_12px_48px_rgba(0,0,0,0.18)] active:scale-95 focus-visible:ring-4 focus-visible:ring-white/50 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100"
        onClick={next}
        disabled={disabled}
        aria-label="Next Image"
      >
        <ChevronRight className="w-6 h-6 sm:w-8 sm:h-8 text-white transition-transform duration-300 group-hover:translate-x-0.5" />
      </button>
    </div>
  )
}
